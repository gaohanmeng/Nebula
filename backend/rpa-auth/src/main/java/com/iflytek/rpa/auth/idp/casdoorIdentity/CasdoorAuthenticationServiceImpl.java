...
public class CasdoorAuthenticationServiceImpl implements AuthenticationService {
    ...
    public User loginWithTenant(String tempToken, String tenantId, HttpServletRequest servletRequest) {
        ...
        try {
            ...
            // 2. 构建Casdoor登录请求（使用用户实际选择的租户/组织）
            CasdoorLoginDto casdoorLoginDto = new CasdoorLoginDto();
            casdoorLoginDto.setApplication(applicationName);
            // 优先使用前端传入的tenantId（用户选择的组织），为空时回退到配置默认值
            casdoorLoginDto.setOrganization(StringUtils.hasText(tenantId) ? tenantId : organizationName);
            ...
            if (loginDto != null) {
                ...
                try {
                    ...
                    if (casdoorUser != null && StringUtils.hasText(casdoorUser.owner)) {
                        ...
                        log.info("从用户信息获取到组织：{}", userOrg);
                    }
                } catch (Exception e) {
                    log.warn("查找用户组织失败，将使用配置默认值：{}", e.getMessage());
                }
            }

            // 优先使用用户实际组织，回退到配置默认值
            String effectiveOrg = StringUtils.hasText(userOrg) ? userOrg : organizationName;
            log.info("使用组织查询租户列表：{}", effectiveOrg);
            return tenantService.getTenantList(effectiveOrg, request);
        } catch (ServiceException e) {
            throw e;
        } catch (Exception e) {
    ...
    public String register(RegisterDto registerDto, HttpServletRequest request) {
        ...
        try {
            ...
            loginDto.setTenantId(StringUtils.hasText(registerDto.getTenantId())
                    ? registerDto.getTenantId() : organizationName);
