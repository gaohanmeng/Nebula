...
拦截除"Nebula RPA"客户端窗口以外的所有鼠标操作，
直到元素保存完成后销毁覆盖窗口。

技术方案: Win32 Layered Window + SetWindowRgn 区域挖洞
- 创建全屏 TOPMOST 透明 Layered Window（alpha=1，几乎不可见）
- 通过 SetWindowRgn 设置窗口区域为 "全屏 - Nebula RPA窗口区域"
- "Nebula RPA"区域无覆盖窗口，点击自然穿透到客户端
- 其他区域有覆盖窗口，点击被拦截吞掉
- 定时器定期刷新区域以适应窗口移动/缩放
...
class BlockOverlay:
    """
    全屏透明覆盖窗口 - 拦截除"Nebula RPA"窗口外的所有鼠标操作。
    ...
    _RPA_WINDOW_KEYWORD = "Nebula RPA"
    ...
    def show(self):
        """显示覆盖窗口，开始拦截所有非"Nebula RPA"区域的鼠标操作"""
        with self._lock:
    ...
    def _find_rpa_window_rects(self) -> list[tuple]:
        """
        查找所有"Nebula RPA"相关窗口的屏幕矩形。

        策略:
        1. 枚举所有可见顶层窗口
        2. 找到标题包含"Nebula RPA"的窗口，记录其进程 ID
